...
const FreeSearchResultCard: React.FC<ModernSearchResultCardProps> = ({ user }) => {
...
export default FreeSearchResultCard;

// Exemplo de uso para exibir múltiplos usuários nos resultados de pesquisa:
/*
import FreeSearchResultCard from "./SearchResultCard";
...
export default function SearchResults() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {users.map((user) => (
        <FreeSearchResultCard key={user.username} user={user} />
      ))}
    </div>
  );
}
