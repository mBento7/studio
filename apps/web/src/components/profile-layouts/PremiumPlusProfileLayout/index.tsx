...
const PremiumPlusProfileLayout: React.FC<{
  user?: UserProfile;
      ...
      <div className="pt-16">
        ...
        <div className="relative z-10">
                ...
                >
                  <Card className="bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-3xl shadow-2xl max-w-md w-full">
                    <div className="text-center">
                      ...
                      >
                        <Avatar className="w-32 h-32 border-4 border-white/20 shadow-xl mx-auto">
                      ...
                      <div className="flex justify-center gap-3 mb-6">
                        {user.socialLinks?.slice(0, 4).map((link) => (
                          <motion.a
                            key={link.id}
                            href={link.url}
                            target="_blank"
                      ...
                      {/* Contact Button */}
                      <Button 
                        className="w-full bg-white text-slate-900 hover:bg-slate-100 font-semibold rounded-full"
                        onClick={handleShare}
                      >
          ...
          <div className="w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/20 dark:border-slate-700/20">
            <div className="container mx-auto px-4">
              <Tabs defaultValue="portfolio" className="w-full">
                <TabsList className="w-full flex justify-center gap-2 bg-transparent shadow-none border-0 py-4">
                  {[
          ...
          {user.reviews && user.reviews.length > 0 && (
            <section className="py-16 px-4 max-w-5xl mx-auto">
              <h3 className="text-2xl font-bold mb-8 flex items-center gap-2"><Star className="w-5 h-5 text-yellow-400" /> Depoimentos</h3>
              <div className="grid md:grid-cols-2 gap-8">
                {user.reviews.map((review) => (
                  <Card key={review.id || review.id} className="p-6 flex flex-col gap-4 bg-white/80 dark:bg-slate-800/80">
                    <div className="flex items-center gap-3">
                      <Avatar className="w-12 h-12 border-2 border-blue-400">
                        <img src={review.authorAvatarUrl} alt={review.authorName} />
                      </Avatar>
                      <div>
                        <div className="font-semibold text-blue-700 dark:text-blue-300">{review.authorName}</div>
                    ...
                    <div className="text-slate-700 dark:text-slate-300 italic">"{review.comment}"</div>
...
export default PremiumPlusProfileLayout;
